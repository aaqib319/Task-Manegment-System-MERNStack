...
const QAFailedTask = ({ tasks, updateTask }) => {
  const [popup, setPopup] = useState(null);

  const getStatusClasses = (status) => {
    return "bg-rose-600"; // QA Failed color
  };
          ...
          tasks.map((task) => (
            ...
            >
              ...
              <div className="w-40 flex justify-end gap-2">
                ...
                <button
                  onClick={() => handleAction("UpdateStatus", task.id || task._id, "in-progress")}
                  className="p-2 rounded-full bg-purple-500 hover:bg-purple-600 transition-colors duration-200"
                  title="Re-assign to Dev (In Progress)"
                >
                  {/* Refresh/Undo icon SVG */}
